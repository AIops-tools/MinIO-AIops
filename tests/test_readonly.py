...
from minio_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_governed_write_is_refused_in_read_only(read_only):
    ...
    def delete_everything(bucket_name: str = "lab-bucket") -> dict:
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def list_things(bucket_name: str = "lab-bucket") -> list:
        return [{"bucket": "thing"}]

    assert list_things() == [{"bucket": "thing"}]
...
def test_medium_risk_write_is_also_refused(read_only):
    ...
    def set_versioning(bucket_name: str = "lab-bucket") -> dict:
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "bucket_delete" in dropped and "set_versioning" in dropped
        assert "bucket_ls" not in dropped and "object_ls" not in dropped
        ...
        assert "bucket_ls" in remaining, "reads must still be exposed"
        assert "capacity_rca" in remaining, "the RCA reads must still be exposed"
    finally:
        registry.clear()
