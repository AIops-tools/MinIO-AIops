...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/minio-aiops` to
  `@zw008/minio-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/minio-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/minio-aiops`. States the `uvx`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/minio-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [minio-aiops, uvx]` — either one suffices — with every variable kept
