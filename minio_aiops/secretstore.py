"""Encrypted secret storage for minio-aiops.

Secrets (MinIO secret keys) are NEVER written to disk in plaintext. They
live in ``~/.minio-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``MINIO_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "minio-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "MINIO_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``MINIO_<TARGET>_PASSWORD``; ``prefix='MINIO_'`` and
