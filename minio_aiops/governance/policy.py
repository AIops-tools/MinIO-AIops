"""Policy engine — rule-based access control for MinIO MCP tools.

Rules are loaded from ``~/.minio-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("minio-aiops.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("MINIO_POLICY_DISABLED") == "1":
            ...
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        ...
                        "require a named approver (MINIO_AUDIT_APPROVED_BY) by "
