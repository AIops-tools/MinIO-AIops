...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``MINIO_RUNAWAY_MAX`` times within ``MINIO_RUNAWAY_WINDOW_SEC`` trips a
...
from minio_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing MinIO-specific; the decorator wraps it into a
