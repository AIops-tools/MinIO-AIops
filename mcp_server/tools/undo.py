...
the SAME governance path as any other call — audited and budget-checked, with
its own risk tier recorded on the audit row. ``undo_apply`` is itself governed.
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    own real risk tier there. Pass dry_run=True to preview the inverse call
